...
def parse_args():
    ...
    parser.add_argument("--data_dir", type=str, default='/voyager/ImageNetS919', help="data folder")
...
def train_epoch(vqvae, loss_fn, dataloader, optimizer_G, optimizer_D, lr_scheduler_G, lr_scheduler_D, progress_bar, rank, args):
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        recon_images, usages, vq_loss, mean_entropy_loss = vqvae(images, masks)  # BLC, C=vocab size

        aeloss, log_dict_ae = loss_fn(vq_loss + mean_entropy_loss, images, recon_images, 0, 
        ...
        discloss, log_dict_disc = loss_fn(vq_loss + mean_entropy_loss, images, recon_images, 1, 
        args.completed_steps, last_layer=vqvae.module.get_last_layer(), split="train")
        ...
        if rank == 0:
            ...
            if args.completed_steps % args.log_interval == 0:
                ...
                image = torch.cat([images, recon_images], dim=0)
...
def save_checkpoint(model, optimizer, args, epoch=None, step=None, ):
    checkpoint = {
    ...
    torch.save(checkpoint, f'{args.output_dir}/checkpoint_step_{step}.pth')
...
def process(rank, world_size, args):
    ...
    disc_in_channels=3,disc_start=20000,disc_weight=0.8,codebook_weight=1.0).to(device)
    loss_fn = DDP(loss_fn)
    ...
    optimizer_G = torch.optim.AdamW(trainable_params_G, lr=args.learning_rate, weight_decay=args.weight_decay)
    optimizer_D = torch.optim.AdamW(trainable_params_D, lr=args.learning_rate, weight_decay=args.weight_decay)
    ...
    for epoch in range(args.starting_epoch, args.num_epochs):
        ...
        if args.save_interval  == 'epoch' and rank == 0:
            ...
            os.makedirs(save_dir, exist_ok=True)
            save_checkpoint(var, optimizer, args, epoch, args.completed_steps)
