...
class VQVAE(nn.Module):
    def __init__(
        ...
        self.mask_quantize: VectorQuantizer2 = VectorQuantizer2(
        ...
        self.filter = torch.nn.Conv2d(self.Cvae, self.Cvae, quant_conv_ks, stride=1, padding=quant_conv_ks//2)
    ...
    # def forward(self, img, msk, ret_usages=False):   # -> rec_B3HW, idx_N, loss
    #     VectorQuantizer2.forward
    #     mh_BChw, musages, mvq_loss, m_entropy_loss = self.mask_quantize(self.quant_conv(self.encoder(msk)), ret_usages=ret_usages)
    #     h_BChw, usages, vq_loss, entropy_loss = self.quantize(self.quant_conv(self.encoder(img))-mh_BChw, ret_usages=ret_usages)
    #     return self.decoder(self.post_quant_conv(h_BChw+mh_BChw)), usages, vq_loss+mvq_loss, entropy_loss

    def forward(self, img, msk, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        VectorQuantizer2.forward
        latent_img = self.quant_conv(self.encoder(img))
        with torch.no_grad():
            latent_mask = self.quant_conv(self.encoder(msk))
        mh_BChw, musages, mvq_loss, _ = self.mask_quantize(latent_mask, ret_usages=ret_usages)

        h1_BChw, usages1, _, _ = self.mask_quantize(latent_img, ret_usages=ret_usages)
        latent_pixel = self.filter(latent_img-h1_BChw)
        h2_BChw, usages2, vq_loss, _ = self.quantize(latent_pixel, ret_usages=ret_usages)
        
        with torch.no_grad():
            recon_msk = self.decoder(self.post_quant_conv(self.filter(mh_BChw)))
        return self.decoder(self.post_quant_conv(self.filter(h1_BChw)+h2_BChw)), recon_msk, usages2, mvq_loss, vq_loss

    def get_last_layer(self):
        return self.decoder.conv_out.weight


    def visualize_ms_f(self, inp):
        ms_f = self.quantize.get_ms_tokens(self.quant_conv(self.encoder(inp)), ret_usages=False)
        ms_img = []
        for f in ms_f:
            img = self.fhat_to_img(f)
            ms_img.append(img)
        return ms_img
