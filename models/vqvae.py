...
class VQVAE(nn.Module):
    ...
    def forward(self, img, ret_usages=False):   # -> rec_B3HW, idx_N, loss
        ...
        h_BChw, usages, vq_loss, entropy_loss = self.quantize(self.quant_conv(self.encoder(img)), ret_usages=ret_usages)
        return self.decoder(self.post_quant_conv(h_BChw)), usages, vq_loss, entropy_loss
