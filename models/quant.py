...
class VectorQuantizer2(nn.Module):
    ...
    def forward(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
        ...
        with torch.autocast(device_type=f_hat.device.type, enabled=False):
        ...
        return f_hat, usages, mean_vq_loss, 0.

    
    def get_entropy_loss(self):
        vocab_hit_probabilities = self.ema_vocab_hit_SV / self.ema_vocab_hit_SV.sum()
        epsilon = 1e-10
        entropy_loss = -torch.sum(vocab_hit_probabilities * torch.log(vocab_hit_probabilities + epsilon))
        return torch.mean(entropy_loss) * self.beta


    def get_ms_tokens(self, f_BChw: torch.Tensor, ret_usages=False) -> Tuple[torch.Tensor, List[float], torch.Tensor]:
    ...
    def idxBl_to_var_input(self, gt_ms_idx_Bl: List[torch.Tensor]) -> List[torch.Tensor]:
        """
        ...
        with torch.autocast(device_type=gt_ms_idx_Bl[0].device.type, enabled=False):
