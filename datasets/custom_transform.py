...
def resize_(image, target, size, max_size=None):
    # size can be min_size (scalar) or (w, h) tuple
    ...
    #print(img_size)
    size = get_size(img_size, size, max_size)
    rescaled_image = F.resize(image, size)

    if target is None:
        return rescaled_image, None
    #print(rescaled_image.shape)
    rescaled_image_size = rescaled_image.shape[-2:][::-1]
    ratios = tuple(float(s) / float(s_orig) for s, s_orig in zip(rescaled_image_size, img_size))
    ratio_width, ratio_height = ratios
